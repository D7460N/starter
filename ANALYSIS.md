# ANALYSIS.md â€” DHCP â†’ starter file reconciliation
...
> **Phase.** Analysis only â€” no code moved. This file is the resumable record.
>
> **Verdict legend**
> - `keep` â€” capability/intent missing from starter; port it in.
> - `merge` â€” same concern already exists in starter; reconcile into the existing file.
> - `drop` â€” obsolete, decorative, inactive, or superseded.
> - `needs-you` â€” intent or desired scope is ambiguous; user decision required (see Questions).
>
> **Compliance flag.** `âš D7460N` = file (usually JS) carries needed intent but
...
`starter` is a clean, compliant **content-display shell**: `app.js â†’ oninput.js
â†’ api.js/storage.js/tour.js`. It fetches a `shell` + page JSON and injects
...
Save/Reset/Delete/Close, field-type inference, APIâ†”UI schema mapping, full
...
fetchâ†’inject, semantic HTML, theming). **The gap is the entire data layer** â€”
...
|---|---|---|---|
| `index.html` | `index.html` | `needs-you` | Different app *shape*. DHCP adds: `<nav><details><summary><section>` grouped filters, `<main>` new-item button + `<input type=search>` filter + dual `<ul>` (header + rows), `<aside>` CRUD `<form><fieldset>` with Delete/Reset/Save labels, SW registration. starter's is the generic radio-nav content shell. All DHCP structures are architecture-compliant and are the **template for a data UI** â€” adopt only if starter is meant to become the management app (see Q1). |
...
| `forms.css` | *(none)* | `keep` âš D7460N | starter has **no form styling**. Needed for any edit form: input/select/textarea, valid/invalid borders, base-select. **But** its button-enable logic keys off `[data-dirty="true"]` â€” forbidden `data-*`. Port styling; re-drive state from `:has(:valid/:invalid)`/`:checked`, not `data-*`. |
...
| `_app.css` | â€” | `drop` | Comments only, no rules. |
...
| `_iframe.css` | â€” | `drop` | iframe styling; no iframes in either app. |
| `_menus.css` | â€” | `drop` | `label[for=â€¦]:hover menu` dropdown; relies on `for`/id, not used; superseded by `<details>` nav. |
| `hero.css` | â€” | `drop` | `/* coming soon */` only. |
| `stars.css` | â€” | `drop` (decorative) | Animated starfield (giant box-shadow). DHCP-specific aesthetic, not loaded. Keep only if the visual is wanted (Q3). |
| `diag.css` | â€” | `drop` (dev-only) | Diagnostic overlay (layout/SR/violation badges). Useful dev tool, not runtime. Optionally preserve as an opt-in dev file (Q3). |

starter-only CSS with no DHCP source (`inputs.css`, `carousel.css`,
`2026.02.28.css`, `*_.css`): out of scope here â€” they're starter's own
...
| `fetch.js` | `api.js` (GET only) | `keep` âš  | **Critical missing capability**: `postJSON/putJSON/deleteJSON`. starter `api.js` only does GET (`fetchJson`). Fold write methods into starter's `api.js` (keeping its logging). |
...
| `inject.js` | `oninput.js` (text-only) | `keep` âš D7460N | Builds data table rows, nav items, and **form inputs typed by rule** (select/toggle/textarea/datetime/text). starter only `createTextNode`s into fixed slots. Needed for data grids/forms. Rewrite away from `innerHTML`/dynamic `import()`/`dispatchEvent`; keep the JSONâ†’element generation intent. |
| `forms.js` | *(none)* | `keep` âš D7460N | **The whole CRUD form lifecycle** (new/edit/save/delete/reset/close, dirty-tracking, validation, row mirroring). No starter equivalent. Heaviest rewrite: uses `.onchange`, `form.dataset.dirty/valid`, console debug â€” all forbidden. Port intent via `oninput` + CSS `:has`/`:checked` state. |
| `rules.js` | *(none)* | `keep` | `inferFieldRules()` â€” derives input type from data values. No starter equivalent; required to auto-generate forms. Compliant (pure function). |
| `schema.js` | *(none)* | `keep` | `ENDPOINT_SCHEMAS` APIâ†”UI field-name mapping + normalize/denormalize. Domain data-layer; needed if those endpoints are used (DHCP-specific â€” confirm endpoints in Q2). |
...
1. **Transport** â€” add write methods to `api.js` (from `fetch.js`); add `config`/`env` constants.
2. **Data shaping** â€” `rules.js`, `schema.js`, `utils.js` (mostly drop-in; pure functions).
3. **Generation** â€” reconcile `inject.js` element-building into the oninput lifecycle.
4. **CRUD UI** â€” rewrite `forms.js` as CSS-state + oninput; add `forms.css` (de-`data-*`) and `fallbacks.css`.
5. **PWA** â€” add `sw.js` with a corrected asset list.
6. **HTML** â€” adopt DHCP's data-UI structure in `index.html` *iff* starter is to become the management app (Q1).

CSS 1:1 pairs (Â§3 `merge` rows) are verdicted at the **concern level**; their
...
2. **Endpoints/domain.** Is the target still the DHCP domain (servers, scopes,
   credentials, audit, â€¦) â€” i.e. keep `schema.js`'s endpoint map â€” or a
...
- [ ] DHCP 1:1 CSS read line-by-line (deferred â€” verdicted by concern).
- [ ] User answers Q1â€“Q4.
- [ ] Port phase (separate, on approval) following Â§5 order.
