...
- [ ] User answers Q1-Q4.
- [ ] Port phase (separate, on approval) following Sec.5 order.| `loading.css` | (inline in `layout.css`) | `merge` | starter already has a loading spinner (`:root:has(article h1:empty)::before`). DHCP's is a `:checked`-driven variant. Keep one. |
